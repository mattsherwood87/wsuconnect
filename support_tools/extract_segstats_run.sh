...
for run_num in $(ls ${ses_num}/segstats/perf); do

idx=`echo $run_num | grep -b -o run- | awk 'BEGIN {FS=":"}{print $1}'`
run=${run_num:idx:6}
echo -e "\t\t$run"

printf "%s,"  "${subj}_${ses}_${run}" >> CorticalMeasuresENIGMA_mean.csv
...
printf "%g," `grep -w ${x} ${ses_num}/segstats/perf/${subj}_${ses}_acq-cbf_task-rest_${run}_proc-flirt_res-hi_space-individual_desc-vol-1-brain_asl.dat | grep -v lobe | awk '{print $6}'` >> CorticalMeasuresENIGMA_mean.csv
