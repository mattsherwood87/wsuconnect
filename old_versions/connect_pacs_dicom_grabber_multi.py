...
class CopyRaw(Process):
    def __init__(self,queue,inDcm):
        Process.__init__(self)
        self.queue =  queue
        self.inDcm = inDcm

    def copy_dcm(self,inDcm):
        ...
        inFilePath = inDcm
        dirs = os.path.dirname(inFilePath)

        #anything after this may fail
        try:
            ...
            sql_table_insert(patientNameSplit[0].replace('-','_') + '_sourcedata',{'fullpath': destFilePath, 'filename':os.path.basename(destFilePath)})
            ...
            df_dir = pd.concat([df_dir,pd.DataFrame.from_dict({'project':[creds.project], 'inDir':[os.path.dirname(dcmDir)]})],ignore_index=True)
            ...
            shutil.copyfile(inFilePath, os.path.join(tmp_destBasePath,os.path.basename(inFilePath)))
            ...
            return pacsFile, df_dir, rawDcm, dirs, currentDateTime


        # Loop over directory recursively
    

        return pacsFile, df_dir, rawDcm, dirs, lastUpdateTime
    
    def run(self):
        # pacsFile, df_dir, rawDcm, dirs, lastUpdateTime = self.copy_dcm(self.inDcm)
        df_dirs = self.copy_dcm(self.inDcm)
        self.queue.put(df_dirs)
        ...
        try:
            ...
            # for path in Path(dcmPath).rglob('*'):
                for filename in files:
                    CopyRaw(queue,os.path.join(root,filename)).start()
                    count += 1
                    ...
                    if count == 120:
                        ...
                        while count > 0:
                            pacsFile, tmp_df_dir, rawDcm, dirs, ts = queue.get()
                            if ts > lastUpdateTime:
                                lastUpdateTime = ts
                            ls_pacsFiles.append(pacsFile)
                            if dirs:
                                if not dirs in ls_dirs:
                                    ls_dirs = dirs
                            if not tmp_df_dir.empty:
                                if not df_dir['inDir'].str.contains(tmp_df_dir['inDir'][0]).any():
                                    df_dir = pd.concat([df_dir,tmp_df_dir],ignore_index=True)
                            if rawDcm:
                                if not rawDcm in ls_rawDcm:
                                    ls_rawDcm.append(rawDcm)
                            
                            count -= 1
                        count = 0
                    # print(count)
            
            #rejoin any lingering jobs
            while count > 0:
                pacsFile, tmp_df_dir, rawDcm, dirs, ts = queue.get()
                if ts > lastUpdateTime:
                    lastUpdateTime = ts
            ...
            if datetime.datetime.now() - lastUpdateTime > datetime.timedelta(minutes=3) and ls_rawDcm: #not df_dir.empty:
                print('Remvoving empty directories from PACS temporary directory')
                for d in ls_dirs:
                ...
                for destFilePath in ls_rawDcm:
                    ...
                    print('Converting DICOMs in ' + destFilePath)
                    ...
                    print('Creating NIfTI data in rawdata directory for sourcedata directory ' + os.path.dirname(destFilePath))
