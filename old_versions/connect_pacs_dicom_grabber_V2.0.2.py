...
#
# Last Modified on 2 July 2024 - add removal of processed participants from the PACS database
        ...
        # for path in Path(dcmPath).rglob('*'):
            ...
            for filename in files:
                ...
                try:
                    ...
                    if not dcmHdr.PatientName.family_name in ls_patientNames:
                        ls_patientNames.append(dcmHdr.PatientName.family_name)
...
#                    sql_table_insert(projectIDs[patientNameSplit[0]]['project'].replace('-','_') + '_sourcedata',d)
        ...
        if datetime.datetime.now() - lastUpdateTime > datetime.timedelta(minutes=5) and ls_rawDcm: #not df_dir.empty:
            #clear PACS database 
            write_log('\tclearing patients from PACS database @ ' + datetime.datetime.now().strftime('%m%d%Y %H:%M:%S'))
            ...
            for patientUID in patientUIDs:
                ...
                if not 'MainDicomTags' in d_patientInfo.keys():
                    ...
                    continue
                if d_patientInfo['MainDicomTags']['PatientName'] in ls_patientNames:
                    RT.DoDelete(URL + '/patients/' + patientUID)
            ls_patientNames = []
