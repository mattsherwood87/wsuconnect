    ...
    try:
        ...
        if 'standard_reference_params' in flirtFullParams:
            ...
            if 'type' in stdImageParams:
                ...
                if stdImageParams['type'] == 'tpl-MNI152NLin2009cAsym':
                    stdImageFile = os.path.join('resshare','templateflow','tpl-MNI152NLin2009cAsym',stdImageParams['file'])
                ...
                if not os.path.isfile(stdImageFile):
                    os.system('echo "WARNING: standard reference file not found"')
                    os.system('echo "/tSkipping standard transformation and/or concatenation"')
        ...
        mainFlirtOutputDir = os.path.join(DATA_DIR,'derivatives','sub-' + st.subject.id,'ses-' + st.subject.fullSesNum,mainParams['output_bids_location'])#create base path and filename for move
        mainBetOutputDir = os.path.join(DATA_DIR,'derivatives','sub-' + st.subject.id,'ses-' + st.subject.fullSesNum,mainParams['output_bids_location'])#create base path and filename for move
        ...
        if refImage and not refImageParams['type'] == 'std':
            ...
            elif refImageParams['input_bids_location'] == 'derivatives' and not refImageParams['type'] == 'fmriprep':
                refImageFile = glob(os.path.join(os.path.dirname(mainBetOutputDir),'**','*' + ref_regexStr + '*'), recursive=True)
            elif refImageParams['input_bids_location'] == 'derivatives' and refImageParams['type'] == 'fmriprep':
                refImageFile = glob(os.path.join(DATA_DIR,'derivatives',f"sub-{st.subject.id}",f"*{ref_regexStr}*"), recursive=True)
        ...
        # **********************************
        # get secondary image to apply transform to
        # **********************************
        ...
        if refImageFile:
            ...
            if os.path.isfile(n_flt.inputs.out_matrix_file) and not overwrite:
                ...
                # if b_fslroi:
                if refImageParams['type'] == 'fmriprep':
                    n_applyMask = pe.Node(interface=fsl.ApplyMask, name='ref_apply_brainmask')
                    n_applyMask.inputs.in_file = refImageFile
                    n_applyMask.inputs.mask_file = refImageFile.replace('preproc','brain').replace('T1w','mask')
                    n_applyMask.inputs.out_file = refImageFile.replace('preproc','brain')

                    workFlow.connect([(n_applyMask,n_flt,[('out_file','reference')])])
                ...
                workFlow.connect([(n_flt,n_pngAppend,[('reference','reference')])])
            ...
            if secImage:
                ...
                workFlow.connect([(n_flt,n_applyXfm,[('reference','reference')])])
                ...
                workFlow.connect([(n_flt,n_pngAppend2,[('reference','reference')])])
